...
def create_tournament_codes(tournament_id, team_size=5, spectator_type='ALL', pick_type='TOURNAMENT_DRAFT',
    ...
    payload = cassiopeia.type.dto.tournament.TournamentCodeParameters(
        team_size, spectator_type, pick_type, map_type, metadata).to_json()
    params = {
        'tournamentId': tournament_id,
        'count': count
    }
    request = (BASE_URL + "code").format(version=cassiopeia.dto.requests.api_versions["tournamentprovider"]) + "?tournamentId=" + str(params['tournamentId']) + "&count=" + str(params['count']) 
